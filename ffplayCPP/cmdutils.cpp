...
void show_help_children(const AVClass *class2, int flags)
{
    ...
    if (class2->option) {
        av_opt_show2(&class2, NULL, flags, 0);
        printf("\n");
    }

    while (child = av_opt_child_class_next(class2, child))
        ...
        *(double *)dst = parse_number_or_die(opt, arg, OPT_DOUBLE, -INFINITY, INFINITY);
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR,
                   "Failed to set value '%s' for option '%s': %d\n",
                   arg, opt, ret);
            return ret;
        }
    }
...
/*
//Another warning free version is below with _dupenv_s()
...
void parse_loglevel(int argc, char **argv, const OptionDef *options)
{
    ...
    if (idx && argv[idx + 1])
        opt_loglevel(NULL, "loglevel", argv[idx + 1]);
    idx = locate_option(argc, argv, options, "report");
	if((_dupenv_s(&env, &sz, "FFREPORT") == 0 && env != nullptr) || idx)
	{
        init_report(env);
        if (report_file) {
            int i;
                         ...
                         const char *arg)
{
	...
	*nb_elems = l->nb_groups;
    //GROW_ARRAY(l->groups, l->nb_groups);
	grow_array(l->groups, sizeof(l->groups), nb_elems, *nb_elems + 1);
                    ...
                    const char *key, const char *val)
{
	...
	*nb_elems = g->nb_opts;
	...
	grow_array(g->opts, sizeof(g->opts), nb_elems, *nb_elems + 1);
...
static void expand_filename_template(AVBPrint *bp, const char *template2,
                                     struct tm *tm)
{
    int c;
...
static int init_report(const char *env)
{
    ...
    while (env && *env) {
        if ((ret = av_opt_get_key_value(&env, "=", ":", 0, &key, &val)) < 0) {
            if (count)
                av_log(NULL, AV_LOG_ERROR,
                       "Failed to parse FFREPORT environment variable: %d\n",
                       ret);
            break;
        }
        if (*env)
	...
	fopen_s(&report_file, filename.str, "w");
    if (!report_file) {
		char buffer[128];
...
static void print_buildconf(int flags, int level)
{
    ...
    while ((conflist = strstr(str, " --")) != NULL) {
        strncpy_s(conflist,3, "~--", 3);
    }

    // Compensate for the weirdness this would cause
    // when passing 'pkg-config --static'.
    while ((remove_tilde = strstr(str, "pkg-config~")) != NULL) {
        strncpy_s(remove_tilde,11, "pkg-config ", 11);
    }

	char *next_token;
    splitconf = strtok_s(str, "~", &next_token);
    av_log(NULL, level, "\n%sconfiguration:\n", indent);
    while (splitconf != NULL) {
        av_log(NULL, level, "%s%s%s\n", indent, indent, splitconf);
        splitconf = strtok_s(NULL, "~", &next_token);
    }
}
                      ...
                      const char *codec_name)
{
	...
	char *env_2 = nullptr;
	size_t sz = 0;
	_dupenv_s(&env_1, &sz, "FFMPEG_DATADIR");
	_dupenv_s(&env_2, &sz, "HOME");

    const char *base[3] = { env_1,
		env_2,
                            /*FFMPEG_DATADIR*/"", };
