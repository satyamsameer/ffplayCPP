...
//All the functions used by ffplay service
#pragma region ff_functions

#if CONFIG_AVFILTER
static int opt_add_vfilter(void *optctx, const char *opt, const char *arg)
{
	//GROW_ARRAY(vfilters_list, nb_vfilters);
	int * nb_elems = nullptr;
	*nb_elems = nb_vfilters;
	grow_array(vfilters_list, sizeof(vfilters_list), nb_elems, *nb_elems + 1);
...
static int VIDEO_packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, int *serial)
{
	MyAVPacketList *pkt1;
	int ret;
...
static int VIDEO_decoder_decode_frame(Decoder *d, AVFrame *frame, AVSubtitle *sub) {
	int ret = AVERROR(EAGAIN);
	...
	for (;;) {
		...
		do {
			...
			if (d->packet_pending) {
				...
				if (VIDEO_packet_queue_get(d->queue, &pkt, 1, &d->pkt_serial) < 0)
					return -1;
			}
			if (d->queue->serial == d->pkt_serial)
				break;
...
static int AUDIO_decoder_decode_frame(Decoder *d, AVFrame *frame, AVSubtitle *sub) {
	int ret = AVERROR(EAGAIN);
...
static int get_video_frame(VideoState *is, AVFrame *frame)
{
	...
	if ((got_picture = VIDEO_decoder_decode_frame(&is->viddec, frame, NULL)) < 0)
		return -1;
...
static int configure_audio_filters(VideoState *is, const char *afilters, int force_output_format)
{
	...
	if (is->audio_filter_src.channel_layout)
		snprintf(asrc_args + ret, sizeof(asrc_args) - ret,
			":channel_layout=0x%llu", is->audio_filter_src.channel_layout);
...
static int audio_thread(void *arg)
{
	...
	do {
		if ((got_frame = AUDIO_decoder_decode_frame(&is->auddec, frame, NULL)) < 0)
			goto the_end;
		...
		if (got_frame) {
			...
			while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
				...
				AVRational tbLocal;
				tbLocal.den = frame->nb_samples;
				tbLocal.num = frame->sample_rate;
				af->duration = av_q2d(tbLocal);
...
#endif
			AVRational tbLocal2;
			tbLocal2.den = frame_rate.den;
			tbLocal2.num = frame_rate.num;
			duration = (frame_rate.num && frame_rate.den ? av_q2d(tbLocal2) : 0);
...
static void seek_chapter(VideoState *is, int incr)
{
	...
	AVRational tbFix;
	tbFix.den = 1;
	tbFix.num = AV_TIME_BASE;// Same value as AV_TIME_BASE_Q
	...
	for (i = 0; i < is->ic->nb_chapters; i++) {
		...
		if (av_compare_ts(pos, tbFix, ch->start, ch->time_base) < 0) {
			i--;
			break;
		}
	}
...
{
	int dLocal = SHOW_MODE_NONE;
	dLocal = (!strcmp(arg, "video") ? SHOW_MODE_VIDEO :
...
#pragma endregion ff_functions

//Function to tokenize string on basis of a character
std::vector<std::string> StringTokenize(const std::string& sInput, const char& delimiterInput,int& iMaxLen)
{
	size_t strStart = 0;
	size_t strEnd = sInput.find_first_of(delimiterInput);
	...
	{
		sLocal = sInput.substr(strStart, strEnd - strStart);
		vOutput.emplace_back(sLocal);
		if (iMaxLen<(int)sLocal.length()) {
			iMaxLen = sLocal.length();
		}
		...
		strStart = strEnd + 1;
		strEnd = sInput.find_first_of(delimiterInput, strStart);
	}
...
//Function to build characters arguments from command string
char** FillOptions(const std::string& cmdInputs,int& argc) {

	int iMaxStrLength = -1;
	std::vector <std::string> v_options = StringTokenize(cmdInputs,' ', iMaxStrLength);

	v_options.insert(v_options.begin(), "EXENAME");//Fix string at start

	const int iLen = (int)v_options.size();
	argc = iLen;
	char ** pParam = (char **)malloc(iLen * sizeof(char *));
	for (int i = 0; i < iLen; i++) {
		pParam[i] = (char *)malloc(iMaxStrLength * sizeof(char));
		memset(pParam[i], '\0', iMaxStrLength * sizeof(char));
	}
	...
	for (int j = 0;j < iLen;j++) {
		s_Local = v_options[j];
		iStrLength = (int)s_Local.size();
		int k = -1;
		for (k = 0;k < iMaxStrLength;k++) {
			if (k<iStrLength) {
				pParam[j][k] = s_Local[k];
			}
			else {
				pParam[j][k] = '\0';
			}
...
#ifdef TAKE_COMMAND_LINE_ARGUMENTS
...
#ifndef TAKE_COMMAND_LINE_ARGUMENTS
	//Provide ffplay command line arguments
	std::string cmdInputs = "-x 300 -y 300 -vcodec h264 -i udp://127.0.0.1:10201";
	int argc = -1;
	char **argv = FillOptions(cmdInputs,argc);
#endif
